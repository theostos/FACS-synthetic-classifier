...
for k, filename in enumerate(os.listdir(folder)):
    ...
    for (x, y, w, h) in face:
        ...
        center_y = y + h//2
        if center_x < 900:
            p0 = (center_x-60, center_y-70)
            p1 = (center_x+ 60, center_y + 70)
            bbox.append((p0, p1))
    ...
    if len(bbox) == 6:
        ...
        if profile_2[0] > profile_3[0]:
            profile_2 = bbox[3]
            profile_3 = bbox[2]
        profile_4, profile_5 = bbox[4], bbox[5]
        if profile_4[0] > profile_5[0]:
            profile_4 = bbox[5]
            profile_5 = bbox[4]

        profiles = [profile_0, profile_1, profile_2, profile_3, profile_4, profile_5]
        profile_names = list_names[:6]

        for ((x0, y0), (x1,y1)), name in zip(profiles, profile_names):
            cv2.imwrite(f"export_main/{name}.png", img[y0:y1, x0:x1])
    else:
        for i, ((x0, y0), (x1,y1)) in enumerate(bbox):
            cv2.imwrite(f"export_main/untitled_{k}_{i}.png", img[y0:y1, x0:x1])
    list_names = list_names[6:]